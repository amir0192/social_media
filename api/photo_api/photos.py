...
from database.photoservice import chage_photo_db, delete_photo_db, get_all_or_exact_photo_db
...
async def change_user_photo(photo_id: int = Body(...), photo_file: UploadFile = Body(...)):
    if photo_file:
        ...
        change_user_photo_db (photo, f'/api/photo_api/photos/{photo_id}.jpg')
    return {'status': 1, 'message': 'Photo has been changed'}
